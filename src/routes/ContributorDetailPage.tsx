...
const ContributorDetailPage = () => {
  ...
  if (!contributions.length) {
    return (
      <Box>
        ...
        <Box sx={{ mt: 2 }}>
          <Link to="/archive/contributors">← Back to contributors</Link>
        </Box>
      </Box>
    );
  }
  ...
  return (
    <Box>
      <Box sx={{ mb: 2 }}>
        <Link to="/archive/contributors" style={{ textDecoration: 'none' }}>
            ...
            <TableBody>
              {contributions.map((record) => (
                <TableRow key={record.id} hover>
                  <TableCell>
                    <Link to={`/archive/recipients/${slugify(record.recipientFullName)}`}>
